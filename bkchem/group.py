...
class group( meta_enabled, area_colored, point_drawable, text_like, child, oasa.graph.vertex):
  ...
  #LOOK type (explicit, implicit, builtin)
  def __get_type( self):
    return self.__type
  ...
  def set_name( self, name, interpret=1):
    self.group_graph = self.intepret_name( name)
    if not self.group_graph:
      return False
    return True


  def intepret_name( self, name):
    return None
  ...
  def read_package( self, package):
    ...
    if ft:
      ...
      self.set_name( package.getAttribute( 'name'))
    # charge
